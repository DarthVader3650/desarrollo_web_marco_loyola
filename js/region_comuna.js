...
const seleccionarRegión = () => {
    let regiónEscogida = document.getElementById("región");

    for (const region of region_comuna.regiones) {
        let option = document.createElement("option");
        option.value = region.nombre;
        option.text = region.nombre;
        regiónEscogida.appendChild(option);
    }
}

const seleccionarComuna = () => {
    let comunaEscogida = document.getElementById("comuna");
    let regionEscogida = document.getElementById("región");
    let regionNombre = regionEscogida.value;

    comunaEscogida.innerHTML = '<option value="">Selecciona una comuna</option>';
    const region = region_comuna.regiones.find(reg => reg.nombre === regionNombre);
    ...
    if (region) {
        for (const comuna of region.comunas) {
            ...
            comunaEscogida.appendChild(option);
        }
    }
}
